...
void main() {
  ...
  test('persistGeneratedArtifact stores artifact and logs delivery/audit actions', () async {
    ...
    expect(artifact.storagePath, contains('org-1/user-1/reports/insp-1'));

    final deliveryActions = await DeliveryRepository(deliveryGateway).listByInspection(
      inspectionId: 'insp-1',
      organizationId: 'org-1',
      userId: 'user-1',
    );
    ...
    final auditEvents = await AuditEventRepository(auditGateway).listByInspection(
      inspectionId: 'insp-1',
      organizationId: 'org-1',
      userId: 'user-1',
    );
  ...
  test('startSecureShare and startDownload issue 15-minute signed links and audit writes', () async {
    final auditGateway = InMemoryAuditEventGateway();
    final deliveryGateway = InMemoryDeliveryActionGateway();
    ...
    expect(actions.map((action) => action.actionType), containsAll(<String>['download_started', 'share_started']));
    ...
    expect(events.map((event) => event.eventType), containsAll(<String>['delivery_download_started', 'delivery_share_started']));
  });
}
